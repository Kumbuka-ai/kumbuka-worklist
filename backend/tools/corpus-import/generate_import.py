...
def emit_header(
    ...
    return f"""\
-- ===========================================================================
-- Corpus import for scope kumbuka (Sprint 178.1)
--
...
def build_reference_targets(row) -> list[str]:
    """Ordered list of item_reference.target strings for one source row.

    Order (dispatch 178.1, §"Referenzen"):
      a) the old id (`CHORE-319`, …) if the source carries one.
      b) the segments of the Ref column, split on ` · `, trimmed,
         empties dropped.
      c) if the Scope column is non-empty, one entry
         `component: <raw tokens>`.
    """
    targets: list[str] = []
    ident = (row.ident or "").strip()
    if ident:
        targets.append(ident)
    targets.extend(split_ref_segments(row.ref or ""))
    scope = (row.scope or "").strip()
    if scope:
        targets.append(f"component: {scope}")
    return targets
...
def emit_body(
    ...
    for a in kumbuka_items:
        ...
        if was_truncated:
            ...
            description: str | None = original_title
        else:
            description = None

        targets = build_reference_targets(row)
        if row.ident and row.ident.strip():
            id_ref_count += 1
        references_per_item.append((new_number, targets))
...
def _assert_reference_targets_fit(kumbuka_items: list[Assignment]) -> None:
    """Bricht laut ab, wenn irgendein item_reference.target die btree-v4-
    Zeilengrenze (2704 Byte) uebersteigt. Nennt die betroffenen Zeilen mit
    Nr, Ident und Byte-Groesse, sodass die Rueckgabe des Sub-Sprints
    genau messbare Vorfaelle zitiert."""
    oversize: list[tuple[int, str, str, str]] = []  # bytes, nr, ident, head
    for a in kumbuka_items:
        for target in build_reference_targets(a.row):
    ...
    lines = [
        f"STOPP (Auftrag 178.1): {len(oversize)} vorgesehene "
        f"item_reference.target-Werte uebersteigen den btree-v4-Zeilendeckel",
        f"({BTREE_V4_MAX_TARGET_BYTES} Byte). Der Deckel sitzt am Index",
        "idx_item_reference_target (tenant_id, scope_id, target) aus V4.",
        "Der Auftrag verbietet Schemaanpassung und Inhaltsverkuerzung. Die",
        "Emission haelt hier an; die Konzept-Seite entscheidet.",
        "",
